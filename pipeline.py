...
class ProductCardGeneration:
    ...
    def _detector_product(self, state: AgentState) -> AgentState:
        ...
        so_chain = (
            self.llm.with_structured_output(DetectProductOutput, strict=True)
    ...
    def _background_generation(self, state: AgentState) -> AgentState:
        ...
        if label == "L":
            base_prompt = PROMPT_GENERATE_IMAGE_L
        elif label == "S":
            base_prompt = PROMPT_GENERATE_IMAGE_S
        else:
            base_prompt = PROMPT_GENERATE_IMAGE_M

        prompt_text = base_prompt.format(product_name=product_name)
    ...
    def _generate_characteristics(self, state: AgentState) -> AgentState:
        ...
        so_chain = (
            self.llm.with_structured_output(OutputLLM, strict=True)
            .with_config({"run_name": "generate_characteristics"})
        )
    ...
    def _generate_headers(self, state: AgentState) -> AgentState:
        ...
        so_chain = (
            self.llm.with_structured_output(HeadersOutput, strict=True)
    ...
    def _generate_specs(self, state: AgentState) -> AgentState:
        ...
        so_chain = (
            self.llm.with_structured_output(SpecsOutput, strict=True)
    ...
    def _generate_description(self, state: AgentState) -> AgentState:
        ...
        so_chain = (
            self.llm.with_structured_output(DescriptionOutput, strict=True)
